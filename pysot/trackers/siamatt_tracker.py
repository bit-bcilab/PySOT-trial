...
from pysot.trackers.base_tracker import change, sz
from pysot.trackers.siamrpn_tracker import SiamRPNTracker


class SiamAttTracker(SiamRPNTracker):
    def __init__(self, cfg, model):
        super(SiamAttTracker, self).__init__(cfg, model)
    ...
    def _mask_post_processing(self, mask):
        target_mask = (mask > self.cfg.TRACK.MASK_THERSHOLD)
    ...
    def track(self, img):
        ...
        center_pos = np.array([self.cfg.TRACK.INSTANCE_SIZE / 2, self.cfg.TRACK.INSTANCE_SIZE / 2], dtype=np.float32)
        ...
        rpn_cx, rpn_cy, rpn_width, rpn_height = self._bbox_clip(rpn_cx, rpn_cy, rpn_bbox[2], rpn_bbox[3],
                                                                [self.cfg.TRACK.INSTANCE_SIZE, self.cfg.TRACK.INSTANCE_SIZE])
        ...
        if self.cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
            stds = np.array(self.cfg.TRAIN.BBOX_NORMALIZE_STDS)
            means = np.array(self.cfg.TRAIN.BBOX_NORMALIZE_MEANS)
        ...
        out_size = self.cfg.TRACK.MASK_OUTPUT_SIZE
        mask = mask.view(out_size, out_size).cpu().data.numpy()

        s = crop_box[2] / self.cfg.TRACK.INSTANCE_SIZE
        ...
        mask_for_show = (mask_in_img > self.cfg.TRACK.MASK_THERSHOLD) * 255

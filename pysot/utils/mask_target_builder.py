...
def proposal_target_layer(cfg, rpn_rois, gt_boxes):
    batch_size = len(gt_boxes)
    gt_boxes = gt_boxes.unsqueeze(1).type(torch.float32)
    ...
    if batch_size > 0:
        ...
        for i in range(batch_size):
            ...
            rois, labels, bbox_targets = single_proposal_target_layer(cfg, i, batch_roi, gt_boxes[i])
        ...
        # single image
        rois, labels, bbox_targets = single_proposal_target_layer(cfg, 0, rpn_rois, gt_boxes)
        return rois.unsqueeze(0), labels.unsqueeze(0), bbox_targets.unsqueeze(0)


def single_proposal_target_layer(cfg, batch, rpn_rois, gt_box):
    """
...
def _sample_rois(cfg, all_rois, gt_box, fg_rois_per_image, rois_per_image):
    """
    ...
    bbox_target = _compute_targets(cfg, rois, gt_box.expand(rois.shape[0], -1))

    return labels, rois, bbox_target


def _compute_targets(cfg, ex_rois, gt_rois):
...
def build_proposal_target_(cfg, rois, gt_boxes):
    rois = rois.detach()
    rois, cls_ind, regression_target = proposal_target_layer(cfg, rois, gt_boxes)
    cls_ind = cls_ind.long().detach()
    return rois, cls_ind, regression_target


def build_mask_target_(cfg, rois, cls_ind, gt_masks):
    pred_boxes = rois
    # crop mask wrt roi
...
def convert_loc_to_bbox_(cfg, loc):

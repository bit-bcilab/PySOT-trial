...
def anchor_target(cfg, target, anchors, size, neg=False):
    ...
    if neg:
        cx = size // 2
        cy = size // 2
        cx += int(np.ceil((tcx - cfg.TRAIN.SEARCH_SIZE // 2) / cfg.ANCHOR.STRIDE + 0.5))
        cy += int(np.ceil((tcy - cfg.TRAIN.SEARCH_SIZE // 2) / cfg.ANCHOR.STRIDE + 0.5))
        l = max(0, cx - 3)
        r = min(size, cx + 4)
        u = max(0, cy - 3)
        d = min(size, cy + 4)
...
class AnchorTarget:
    ...
    def __call__(self, target, size, neg=False):
        anchor_num = len(self.cfg.ANCHOR.RATIOS) * len(self.cfg.ANCHOR.SCALES)
        ...
        if neg:
            ...
            cx = size // 2
            cy = size // 2
            cx += int(np.ceil((tcx - self.cfg.TRAIN.SEARCH_SIZE // 2) / self.cfg.ANCHOR.STRIDE + 0.5))
            cy += int(np.ceil((tcy - self.cfg.TRAIN.SEARCH_SIZE // 2) / self.cfg.ANCHOR.STRIDE + 0.5))
            l = max(0, cx - 3)
            r = min(size, cx + 4)
            u = max(0, cy - 3)
            d = min(size, cy + 4)
            ...
            neg, neg_num = select(np.where(cls == 0), self.cfg.TRAIN.NEG_NUM)
            cls[:] = -1
            cls[neg] = 0
        ...
        pos = np.where(overlap > self.cfg.TRAIN.THR_HIGH)
        neg = np.where(overlap < self.cfg.TRAIN.THR_LOW)
...
if __name__ == '__main__':
    a = AnchorTarget()
    cls, delta, delta_weight, overlap = a([100, 150, 160, 180], 25)
    pass

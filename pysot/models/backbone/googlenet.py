...
class Inception3(nn.Module):
    ...
    def forward(self, x, bbox=torch.tensor([])):
        ...
        if len(bbox):
            ...
            stride = 8  # cfg.BACKBONE.STRIDE
            offset = 13  # cfg.BACKBONE.OFFSET

            mask = torch.zeros(x.shape[0], 1, x.shape[-2], x.shape[-1]).float().cuda()
            roi = torch.round((bbox + 1 - offset + stride / 2) / stride - 1).int()

            for i in range(x.shape[0]):
                mask[i, :, max(0, roi[i][1]): (min(roi[i][3], 12)), max(0, roi[i][0]): (min(roi[i][2], 12))] = 1
            x *= mask
        else:
            crop_pad = 4  # self.cfg.BACKBONE.CROP_PAD
